...
*   **参数配置**: **请注意，像模型选择 (例如切换 Gemini 不同版本)、温度 (Temperature)、输出长度限制等参数，需要您直接在 AI Studio 页面的右侧设置面板中进行调整。本代理服务器目前不处理或转发这些通过 API 请求传递的参数。** 您需要预先在 AI Studio Web UI 中设置好所需的模型和参数。
